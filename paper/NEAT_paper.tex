...
The code can be used to robustly calculate uncertainties on the derived abundances, via a Monte Carlo technique.  We show that for typical observational data, this approach is superior to analytic estimates of uncertainties, which inherently assume normally distributed uncertainties which are small relative to the quantity being measured.  These assumptions can break down severaly for low signal-to-noise data.

By using different compilations of atomic data, extinction law parametrizations, and ``zone" methodologies, we investigate the magnitude of the systematic uncertainties arising from these choices, compared to the magnitude of the statistical uncertainties, for a variety of real and synthetic data.  We find that the most important systematic uncertainty is (what?).
\end{abstract}
...
The first point is a matter of convenience but none the less one which discourages many authors from even attempting to propagate uncertainties.  The second and third points are clearly violated in many or most real astrophysical observations, by virtue of which the uncertainties estimated using analytical techniques will be unreliable.

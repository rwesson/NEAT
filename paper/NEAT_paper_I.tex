...
R is known to vary substantially along different sightlines (eg ref1, ref2), but estimating it for particular objects is generally impractical and instead, it is commonly assumed to equal 3.1.  We investigate the effect of an uncertainty in this value by comparing analyses in which R is fixed to be 3.1, and in which R is drawn from a Gaussian distribution with $\mu$=3.1 and $\sigma$=0.15.

To assess the effect of this uncertainty, we analysed the emission line measurements of NGC 7026 presented by Wesson et al. (2005), including the line flux uncertainties which were not published in that paper.  We chose this object as it is significantly reddened (c(H$\beta$)=1.0), and has many very well detected lines in its spectrum (142 lines measured, 120 with SNR$>$3 and 65 with SNR$>$10).  This combination should maximise the effect of an uncertainty on R, relative to the effect of the line flux measurement uncertainties.

The results of the investigation are shown in Figures x and y.  We find that including the effect of an uncertainty in R has a noticeable but hardly significant effect on abundances derived from individual emission lines.  However, once the various emission lines are averaged, and the ionic abundances used as the basis for estimated of the total elemental abundances, there is no noticeable increase in the uncertainty on the total elemental abundances.  We therefore consider it reasonable in future emission line analyses to consider R to be equal to 3.1, unless there is a direct measurement showing a difference greater than 0.15 along the particular line of sight being considered.

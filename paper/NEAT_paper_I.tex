...
Almost all the atomic data used by the code is stored externally in plain text files, so that the user can easily change the atomic data being used without needing to recompile the code.  The only exception to this is that hydrogen recombination data from Hummer \& Storey (1995) is used throughout.  XXXX justify this
